...
export default function Download() {
    ...
    return (
        <div className="w-full min-h-screen bg-stone-800 flex flex-col items-center justify-center gap-5 sm:gap-10 px-5">
            <h1 className="text-3xl sm:text-5xl text-stone-200 font-medium">Download</h1>
            {files.length > 0 ? <>
                    <div className="w-full md:w-1/2 flex flex-col border border-stone-200 rounded-lg">
                        {files.length > 0 && (
                            files.map((file, index) => <div key={index}
                                                            className={`w-full p-3 flex items-center justify-between border-y border-stone-200 ${(index === files.length - 1) && "border-b-transparent"} ${(index === 0) && "border-t-transparent"}`}>
                                <p className="text-stone-200 line-clamp-1">{file.filename}</p>
                                <button onClick={() => downloadFile(file)} aria-label="download-button"
                                        className="btn-class">
                                    <IoMdDownload/></button>
                            </div>)
                        )}
                    </div>
                    <button onClick={() => downloadAllAsZip(files)} className="btn-class">
                        <p>Download All</p>
                        <IoMdDownload/>
                    </button>
                </>
                : <MoonLoader color="#e7e5e4" loading={true} cssOverride={{
                    display: "block",
                    margin: "0 auto",
                }} size={30}/>}
        </div>
